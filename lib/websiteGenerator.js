...
class WebsiteGenerator {
  ...
  async prompt () {
    ...
    const shouldGenerateCSS = await getCSSPreference(this.ui)
    await this.generate(name, authorName, shouldGenerateJS, shouldGenerateCSS)
    this.ui.close()
  }
...
const getCSSPreference = (readline) => {
  return new Promise((resolve) => {
    readline.question('Create CSS? (Y/N)', (answer) => {
      resolve(answer === 'Y')
    })
  })
}
